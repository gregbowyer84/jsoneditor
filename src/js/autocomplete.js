		...
		var p = {
			...
			refresh: function (token, array) {
				...
				for (var i = 0; i < array.length; i++) {
					...
					if ((config.caseSensitive && array[i].indexOf(token) !== -1)
						|| (!config.caseSensitive && array[i].toLowerCase().indexOf(token.toLowerCase()) === -1)) { continue; }
	...
	var rs = {
		...
		repaint: function (element) {
			...
			for (var i = 0; i < optionsLength; i++) {
				...
				if ((!config.caseSensitive && opt.toLowerCase().indexOf(token.toLowerCase()) !== -1)
